...
public class SystemBarUtil {
    ...
    public static void setBlackBar(Window window, @ColorInt int statusBarColor) {
        if (isFlyme4Later()) {
            setStatusBarDarkModeForFlyme4(window, false);
            tintStatusBar(window, statusBarColor, 0f);
        } else if (isMIUI6Later()) {
    ...
    public static void setWhiteBar(Window window, @ColorInt int statusBarColor) {
        if (isFlyme4Later()) {
            tintStatusBar(window, statusBarColor, 0f);
            setStatusBarDarkModeForFlyme4(window, true);
        } else if (isMIUI6Later()&&Build.VERSION.SDK_INT > Build.VERSION_CODES.KITKAT) {
            tintStatusBar(window, statusBarColor, 0f);
            setStatusBarDarkModeForMIUI6(window, true);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            tintStatusBar(window, statusBarColor, 0f);
    ...
    public static void setLightBarDarkMode(Window window, @ColorInt int statusBarColor) {
        setStatusBarDarkMode(window);
        if (isFlyme4Later()) {
            tintStatusBar(window, statusBarColor);
        } else if (isMIUI6Later()) {
          tintStatusBar(window, statusBarColor);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
           tintStatusBar(window, statusBarColor);
